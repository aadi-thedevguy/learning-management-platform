...
export const updateLessonCompleteStatus = createServerFn({ method: "POST" })
  .validator(z.object({ lessonId: z.string(), complete: z.boolean() }))
  .handler(async ({ data }) => {
    try {
...
export const getLesson = createServerFn()
  .validator(z.object({ courseId: z.string(), lessonId: z.string() }))
  .handler(async ({ data }) => {
    const { userId, role } = await getCurrentUser();

    return getCached(
      `lesson:${data.lessonId}:${userId ?? "anonymous"}`,
      [getLessonIdTag(data.lessonId), getUserCourseAccessUserTag(userId ?? "")],
      async () => {
        ...
        const canUpdateCompletionStatus =
          await canUpdateUserLessonCompleteStatus(userId, lesson.id);
...
export const getIsLessonComplete = createServerFn()
  .validator(z.object({ userId: z.string(), lessonId: z.string() }))
  .handler(async ({ data }) => {
