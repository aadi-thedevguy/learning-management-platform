...
const createLessonFn = createServerFn({ method: "POST" })
...
const updateLessonFn = createServerFn({ method: "POST" })
  .validator(z.object({ id: z.string(), values: lessonSchema }))
  .handler(async ({ data }) => {
    try {
...
const deleteLessonFn = createServerFn({ method: "POST" })
...
const updateLessonOrdersFn = createServerFn({ method: "POST" })
  .validator(z.object({ lessonIds: z.array(z.string()) }))
  .handler(async ({ data }) => {
    try {
      if (
